...
def correlated_displacements(rho, l_x, l_a, size):
    ...
    The displacements arrays must be of the same size.
    """
    h_size = size//2
    l_x = np.array(l_x)
    l_a = np.array(l_a)
    ...
    max_index = 0
    step_sum = 0
    # The displacements generated by the equation above doesn't provide us
    # steps of integer sizes and can overtake the lattice limit.
    for dy in l_y:
        step = round(max(0,dy))
